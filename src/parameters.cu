...
void compute_state(thrust::complex<double>* d_myu, Params& params) {
    // Allocate memory for A and N_hat once
    ...
    // Initialize A_0 with complex normal distribution
...
void compute_state_cpp(std::complex<double>* d_myu, Params& params) {
    thrust::complex<double>* thrust_d_myu = reinterpret_cast<thrust::complex<double>*>(d_myu);
    compute_state(thrust_d_myu, params);  // Call the original compute_state
}

void compute_state_raw(double* d_myu_real, double* d_myu_imag, Params& params) {
    thrust::complex<double>* d_myu = new thrust::complex<double>[params.Nx * params.Ny];

    // Reconstruct thrust::complex from real and imag parts
    for (int i = 0; i < params.Nx * params.Ny; ++i) {
        d_myu[i] = thrust::complex<double>(d_myu_real[i], d_myu_imag[i]);
    }

    // Call the original compute_state with thrust::complex
    compute_state(d_myu, params);
